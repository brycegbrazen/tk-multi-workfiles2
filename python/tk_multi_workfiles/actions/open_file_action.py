    ...
    def _do_copy_and_open(
        ...
        if not new_ctx == self._app.context:
            try:
                ...
                QtGui.QMessageBox.critical(
                    parent_ui,
                    "Failed to change the work area",
                    "Failed to change the work area to '%s':\n\n%s\n\nUnable to continue!"
                    % (new_ctx, e),
                )
                self._app.log_exception(
                    "Failed to change the work area to %s!" % new_ctx
                )
                return False
...
class CopyAndOpenInCurrentWorkAreaAction(OpenFileAction):
    ...
    def _open_in_current_work_area(
        ...
        if not dst_work_area.work_template:
            # should never happen!
            app.log_error(
                "Unable to copy the file '%s' to the current work area as no valid "
        ...
        context_fields = dst_work_area.context.as_template_fields(
        ...
        if sandbox_user_name is not None:
            msg += " is in %s's Work Area (%s)." % (
                sandbox_user_name,
                src_work_area.context,
            )
        else:
        ...
        answer = QtGui.QMessageBox.question(
            parent_ui,
            "Open file in current Work Area?",
            msg,
            QtGui.QMessageBox.Yes | QtGui.QMessageBox.Cancel,
        )
        ...
        try:
            ...
            app.log_error(
                "Unable to copy the file '%s' to the current work area as Toolkit is "
